document.addEventListener("DOMContentLoaded", () => {
    const propertySearchInputEnter = document.getElementById(
        "propertySearchInput"
    );

    propertySearchInputEnter.addEventListener("keydown", function (event) {
        if (event.key === "Enter") {
            e.preventDefault();
        }
    });
    ...
    function renderProperties() {
        ...
        propertiesData.forEach((property) => {
            ...
            propertyNameSpan.classList.add(
                "ml-[0.59rem]",
                "relative",
                "bottom-[0.125rem]"
            );
    ...
    function renderBool(property, div, trueInput) {
        ...
        const options = document.createElement("ul");
        options.classList.add("hidden", "w-[11rem]", "flex", "gap-1", "grid");
    ...
    function propertyMultiSelectControl(
        option,
        input,
        id,
        trueInput,
        ...
        if (
            selectedOptionsContainer.querySelector(
        ...
        removePropertyIcon.classList.add(
            "select-none",
            "w-[0.75rem]",
            "h-[0.75rem]",
            "hover:cursor-pointer"
