                    ...
                    <div x-data="{ open: false, selectedProperty: '' }" class='relative flex items-center justify-start text-left right-6'>
                        ...
                        <div class='flex justify-center'>
                            <div x-cloak x-show='open'
                                ...
                                <div class='items-center border-none overflow-y-auto' id='propertyLists2'></div>
                            </div>
                        </div>
                    </div>
...
<script>
    ...
    function searchPropertyTitles(propertySearchText) {
        variationsTitleData.forEach((property) => {
    ...
    const searchVariationPropertyTitles = document.getElementById('searchVariationPropertyTitles');

    if (searchVariationPropertyTitles) {
        searchVariationPropertyTitles.addEventListener('input', () => {
            const propertySearchTitles = searchVariationPropertyTitles.value.trim();
    ...
    const clickedVariationProperty = [];

    function renderProperties(property) {
        // Check if property is already clicked
        const isClicked = clickedVariationProperty.some(prop => prop.id === property.id);
        // If not already clicked, add it to clickedVariationProperty
        if (!isClicked) {
            clickedVariationProperty.push(property);
        }
        
        const createProdPropertyList2 = document.getElementById('createProdPropertyList2');
        createProdPropertyList2.innerHTML = ''; // Clear existing list
        createProdPropertyList2.classList.add('basic:max-h-[16rem]', 'hd:max-h-[22.5rem]', 'uhd:max-h-[27rem]');
        
        // Render all clicked properties
        clickedVariationProperty.forEach(prop => {
            ...
            delPropBtn.addEventListener('click', (event) => {
                ...
                clickedVariationProperty.splice(prop, 1);
                // add removed prop back to propertyTitles
                variationsTitleData.push(prop);
                li.remove();
            ...
            createProdPropertyList2.appendChild(li);
    ...
    const propertyTitleList = document.getElementById('propertyLists2');
    ...
    variationsTitleData.forEach(property => {
        ...
        li.addEventListener('click', () => {
            ...
            variationsTitleData.splice(property, 1);
            li.remove();
        });
    ...
    let blurVariationMultiDelayTimer;
    function blurmultiSelect(options) {
        clearTimeout(blurVariationMultiDelayTimer);
        blurVariationMultiDelayTimer = setTimeout(() => {
            options.classList.add('hidden');
        }, 200);
    }

    let BlurVariationDelayTimer;
        function blurSingleSelect(options) {
        clearTimeout(BlurVariationDelayTimer);
        BlurVariationDelayTimer = setTimeout(() => {
            options.classList.add('hidden');
        }, 200);
    }
